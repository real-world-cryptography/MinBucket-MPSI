...
namespace apsu {
    ...
    namespace receiver {
        namespace {
            ...
            */
                ...
                )
            {
                ...
                vector<oc::block> item_table;
                item_table.reserve(std::distance(begin, end));
                vector<vector<oc::block>> item_hash(params.table_params().table_size);
            ...
            void my_preprocess_unlabeled_data(
                const vector<HashedItem>::const_iterator begin,
                const vector<HashedItem>::const_iterator end,
                const PSUParams &params,
                int serial_number
                )
            {
                ...
                STOPWATCH(recv_stopwatch, "my_preprocess_unlabeled_data");
                APSU_LOG_DEBUG(
                    "Start preprocessing " << distance(begin, end) << " unlabeled items");
                ...
                item_table.reserve(std::distance(begin, end));
                vector<vector<oc::block>> item_hash(params.table_params().table_size);
                //vector<vector<int>> index_co(params.table_params().table_size);
                vector<vector<pair<size_t, size_t>>>item_hash_indices(params.table_params().table_size);
                std::cout<<"params.table_params().table_size"<<params.table_params().table_size<<std::endl;
                ...
                // use the result to parallelize the work of inserting the items into BinBundles.
                vector<pair<AlgItem, size_t>> data_with_indices;
                for (auto it = begin; it != end; it++,item_idx++) {
                    const HashedItem &item = *it;
                    oc::block b(item.value());
                    item_table.emplace_back(b);
                    ...
                    for (auto location : all_locations(hash_funcs, item)) {
                        ...
                        // size_t bin_idx = location * bins_per_item;
                        oprf_in[location].emplace_back(item);
                        item_hash[location].emplace_back(b);
                        item_hash_indices[location].emplace_back(item_idx, location);
                        // Store the data along with its index
                        // data_with_indices.emplace_back(make_pair(alg_item, bin_idx));
                    }
                }

                std::string serialnum =std::to_string(serial_number);

                std::string outFileName = "./hash_result/receiver_1_"+serialnum;
                ...
                outFile.write((char*)item_table.data(), sizeof(oc::block)*(item_table.size()));
            
                int flag = 0;
                outFile.write((char*)(&flag), sizeof(int));

                outFile << item_hash_indices.size() << "\n";
                for (const auto &inner : item_hash_indices) {
                    outFile << inner.size() << "\n";
            ...
            */
            vector<pair<AlgItem, size_t>> preprocess_unlabeled_data(
                const HashedItem &item, const PSUParams &params, coproto::AsioSocket dbsocket,int serial_number)
            {
                vector<HashedItem> item_singleton{ item };
                return preprocess_unlabeled_data(
                    item_singleton.begin(), item_singleton.end(), params,dbsocket,serial_number);
            }
        ...
        void ReceiverDB::insert_or_assign(const vector<pair<Item, Label>> &data,int serial_number)
        {
            if (stripped_) {
        ...
        void ReceiverDB::insert_or_assign(const vector<Item> &data,int serial_number)
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot insert data to a stripped ReceiverDB");
                throw logic_error("failed to insert data");
...
// TO DO : 
            ...
            vector<pair<AlgItem, size_t>> data_with_indices =
                preprocess_unlabeled_data(hashed_data.begin(), hashed_data.end(), params_,DBSocket,serial_number);
            //------------------------------------------receiver's simple hash end---------------------------------------------------
        ...
        void ReceiverDB::my_insert_or_assign(const vector<Item> &data,int serial_number)
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot insert data to a stripped ReceiverDB");
                throw logic_error("failed to insert data");
...
// TO DO : 
            ...
            my_preprocess_unlabeled_data(hashed_data.begin(), hashed_data.end(), params_,serial_number);
            //------------------------------------------receiver's simple hash end---------------------------------------------------
            APSU_LOG_INFO("Finished inserting " << data.size() << " items in ReceiverDB");
        }

        void ReceiverDB::remove(const vector<Item> &data,int serial_number)
        {
            if (stripped_) {
...
// TO DO : 
        ...
        Label ReceiverDB::get_label(const Item &item,int serial_number) const
        {
            if (stripped_) {
            ...
            AlgItem alg_item;
            size_t cuckoo_idx;
            tie(alg_item, cuckoo_idx) = preprocess_unlabeled_data(hashed_item, params_,DBSocket,serial_number)[0];
