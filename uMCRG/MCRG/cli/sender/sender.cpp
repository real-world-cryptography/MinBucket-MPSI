...
void Write_sender_item(const vector<string> &orig_items);
...
int remote_query(const CLP &cmd)
{
    ...
    // std::cout<<"orig_items[0]   "<<orig_items[0]<<std::endl;
    // std::cout<<"orig_items[1]   "<<orig_items[1]<<std::endl;

    Write_sender_item(orig_items);
    ...
    try {
        ...
        sender.request_query(items_without_OPRF, orig_items,cmd.total_parties());
...
void Write_sender_item(const vector<string> &orig_items)
{
    std::string itemsfilepath = "../mcrg_result/orig_item";
    std::ofstream itemsfile(itemsfilepath);
    if (!itemsfile.is_open()) {
        std::cerr << "Error: Unable to open file " << itemsfilepath << std::endl;
    }
    itemsfile << orig_items.size() << std::endl;
    for (const auto& item : orig_items) {
        itemsfile << item << std::endl;
    }
    itemsfile.close();
}
