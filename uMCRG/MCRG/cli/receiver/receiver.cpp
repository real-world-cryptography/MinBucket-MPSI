...
shared_ptr<ReceiverDB> try_load_csv_db(const CLP &cmd,coproto::AsioSocket receiversocket)
{
    ...
    return create_receiver_db( *db_data, move(params), cmd.nonce_byte_count(), cmd.compress(),receiversocket,cmd.serial_number());
}
...
int start_receiver(const CLP &cmd)
{
    ...
    auto ReceiverKKRTSocket = coproto::asioConnect("localhost:" + std::to_string(1212 + cmd.serial_number() * 100), true);
    ...
    dispatcher.serial_number=cmd.serial_number();
    ...
    )
{
    ...
    if (holds_alternative<CSVReader::UnlabeledData>(db_data)) {
        try {
            ...
            receiver_db->setSocket(ReceiverSocket);
            receiver_db->set_data(get<CSVReader::UnlabeledData>(db_data),serial_number);
